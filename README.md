...
> **Programme principal documenté** : `mbd_simple-multi-process_v3_5.py`
> **Version courante** : V3.5
> **Langage** : Python 3.10+
> **Domaine** : essais vibratoires — personnalisation d'environnement mécanique

> **Nouveautés V3.5** (contrôle des formules vs documentation) :
> - Nouvelle option **`METHODE_PROJECTION='gev_domaines'`** : projection SRE
>   par max-stabilité GEV (h figé à 0), discriminée selon les 3 domaines
>   Gumbel / Fréchet / Weibull négative (Colin 2023 §4.1) — cf. §4.10.
> - **Un sous-dossier de résultats par calcul** (`AAAAMMJJ_HHMMSS_<fichier>_<paramètres>`)
>   et nom du fichier analysé (≤35 caractères) dans les titres HTML.
> - Diagramme τ₃/τ₄ : limite basse de validité corrigée τ₄=(5τ₃²−1)/4
>   ([2] eq. 15) ; courbes h=−1 (GLO), h=0 (GEV), h=1 (GPD) étiquetées.
> - **MSDI corrigé** suivant NF X50-144-3 §C.7 / CFM 2025 §3.1 : *Mean Square
>   Deviation Index*, MSDI(%) = (100/m)·Σ[(X_exp − X_loi)/X_exp]² sur les m
>   points > moyenne (queue droite, positions Cunnane a=0.4). L'ancien calcul
>   (écart signé moyen ECDF−CDF, sans carré) ne correspondait pas à la norme.
...
```
┌───────────────────────────────────────────────────────────────────┐
│  ENTRÉE : signal CSV  (t, ẍ)  ─  fs ≈ 10,0 kHz typique            │
└───────────────────────────────────────────────────────────────────┘
              │
              ▼
┌────────────────────────────────────┐
...
### 4.10 Projection longue durée : puissance F^M vs 3 domaines GEV

Deux méthodes de projection du SRE sont disponibles (`METHODE_PROJECTION`) :

1. **`'puissance'`** (défaut, historique) — élévation directe à la puissance M
   de la CDF de la loi sélectionnée (`LOI_AJUSTEMENT`) :
   `SRE_proj = PPF(α^(1/M))` avec `M = Occ(j)·T_v/T_b` ([1] §C.8-C.9, [2] eq. 35).

2. **`'gev_domaines'`** — alternative évoquée par B. Colin ([2] §4.1) : la loi
   locale est ré-ajustée en **figeant h = 0** (famille **GEV**, sous-famille de
   Kappa4 ; `g_r = r^(−k)·Γ(1+k)`, eq. 20.2). Le paramètre de forme k* est
   racine de l'unique équation `τ3_GEV(k) = t3` (1 inconnue au lieu du système
   2×2 en (k,h) — plus stable), puis (ξ*, α*) par les eq. 33-34 avec h=0.
   La projection exploite la **max-stabilité exacte de la GEV**, en
   discriminant les 3 domaines d'attraction de Fisher-Tippett / Gnedenko :

   | Domaine | Condition | Loi asymptotique de Z_sup | Paramètres projetés |
   |---------|-----------|---------------------------|---------------------|
   | EV1 | k ≈ 0 | **Gumbel** (queue fine) | ξ_M = ξ + α·ln(M), α_M = α |
   | EV2 | k < 0 | **Fréchet** (queue épaisse) | ξ_M = ξ + (α/k)(1 − M^−k), α_M = α·M^−k |
   | EV3 | k > 0 | **Weibull négative** (queue bornée) | idem EV2 (même formule, continue en k) |

   puis `SRE_α = ξ_M + (α_M/k)·(1 − (−ln α)^k)` (limite Gumbel pour k≈0).
   Strictement équivalent à `PPF_GEV(α^(1/M))` mais en **forme close** :
   pas d'élévation α^(1/M) → 1, donc pas de perte de précision/clip pour M
   très grand. Le SRE *à la durée du signal* reste celui de `LOI_AJUSTEMENT` ;
   seule la projection change. Le domaine retenu par f₀ (classe dimensionnante)
   est exporté dans `SRE_Projection_*.csv` (colonne `GEV_domaine`) ; le seuil
   d'étiquetage Gumbel est `GEV_GUMBEL_K_TOL` (défaut 0.01, diagnostic
   uniquement — la formule de projection est continue en k).
...
Modifier les constantes en tête du fichier `mbd_simple-multi-process_v5.py` (SECTION 1), puis :

```bash
python mbd_simple-multi-process_v3_5.py
```

À chaque calcul, le programme crée un **sous-dossier dédié** dans `OUTPUT_FOLDER` (défaut `mbd_simple_output/`) :

```
<AAAAMMJJ_HHMMSS>_<nom du fichier ≤35 car.>_f<min>-<max>_Q<Q>_Tb<Tb>_b<b>_T<Tproj>_a<alfa>
ex. : 20260612_231054_signal_roulage_piste_f1-500_Q10_Tb1_b8_T36Ms_a0p9/
```

contenant les CSV, le sidecar JSON des paramètres et les rapports HTML du run. Le nom du fichier analysé (tronqué à 35 caractères) apparaît également dans le titre des rapports HTML et dans le cadre de paramètres.
...
Programme court réutilisant intégralement `main()` du module principal : il
génère un signal d'excitation puis lance le calcul MBD complet avec **exactement
les mêmes visuels et la même sortie** (rapports HTML, CSV suffixés, sidecar
JSON). `python mbd_demo_v1.py` (config en tête de fichier, sections A à D).

**Toutes les options de calcul** du module principal sont exposées et passées au
pipeline (loi d'ajustement, Q/T_b, spectre f₀, Cunnane, SRX α, SDF, projection /
GEV, Quality Gate IID, multiprocess…). Le démonstrateur **applique ses surcharges
au niveau module et enregistre le module principal dans `sys.modules`**, de sorte
que `USE_MULTIPROCESS=True` est sûr même sous *spawn* (Windows) : les workers
héritent de tous les réglages, y compris `LOI_AJUSTEMENT`.

**Trois familles de signaux** (paramètre `MODE_SIGNAL`) :

| `MODE_SIGNAL` | Description | Paramètres dédiés |
|---------------|-------------|-------------------|
| `'stationnaire'` | PSD plate + ZMNL Hermite (CFM 2025 §4.3), piloté par `KURTOSIS`/`SKEWNESS` (K=3 → gaussien, K>3 → non-gaussien) | `DUREE`, `RMS_CIBLE`, `KURTOSIS`, `SKEWNESS` |
| `'phases'` | Concaténation de segments stationnaires de statistiques distinctes → instationnarité **par blocs** | `PHASES = [(durée, rms, kurt, skew), …]` |
| `'enveloppe'` | Signal de base modulé par une enveloppe RMS(t) → instationnarité **continue** | `ENVELOPPE` (`'sinus'`/`'marche_alea'`), `RMS_MIN`, `RMS_MAX`, `PERIODE_ENV` |

Un **bloc de paramètres de classification indépendant** (SECTION B :
`N_CLUSTERS`, `AUTO_SELECT_K`, `K_RANGE`, `MIN_SAMPLES_PER_CLUSTER`,
`FEATURE_FLAGS`) est réservé aux **signaux non-stationnaires** : en mode
`'phases'`/`'enveloppe'`, fixer `N_CLUSTERS` ≈ nombre de régimes (ou
`AUTO_SELECT_K=True`) et activer les features discriminantes
(`variance`/`rms`, `kurtosis`/`crest_factor`, `dominant_freq`…). En
`'stationnaire'`, garder `N_CLUSTERS=1`.

À chaque exécution, toute la sortie du module principal atterrit dans son
**sous-dossier horodaté indépendant** (cf. §5.3) ; la démo y range en plus le
**signal généré** (CSV unique par run) et un **`RESUME_demo.txt`** (config du
run, statistiques RMS/skew/kurtosis atteintes — globales et par phase — et liste
des fichiers produits), puis affiche une synthèse console (pics SRC/SRE et
verdict IID).
...
| `DELTA_F0` | float | 1 | **Pas en fréquence (Hz)**. `num_f0 = round((F0_MAX-F0_MIN)/DELTA_F0) + 1`. Ex. 0.5 → deux fois plus de points. (Corrigé en V3.5 : l'ancienne formule multipliait par ΔF0.) |
...
| `ALFA_PROJECTION` | float | 0.9 | Probabilité de **non-dépassement** projetée (risque α de la norme = 1 − cette valeur). 0.9 = 90% non-dépassé. Plage **0.5–0.999** |
| `METHODE_PROJECTION` | str | `'puissance'` | `'puissance'` = F^M sur la loi `LOI_AJUSTEMENT` (historique) ; `'gev_domaines'` = max-stabilité GEV à h=0, discrimination Gumbel/Fréchet/Weibull nég. ([2] §4.1) — cf. §4.10 |
| `GEV_GUMBEL_K_TOL` | float | 0.01 | (expert) Seuil \|k*\| d'étiquetage du domaine `gumbel` — diagnostic uniquement, la formule de projection est continue en k |
...
Tous dans le **sous-dossier de run** `OUTPUT_FOLDER/<AAAAMMJJ_HHMMSS>_<fichier ≤35 car.>_<infos calcul>/` (un dossier par calcul, cf. §5.3), suffixés par un **tag descriptif** (ex. `v3p5_f1-500_Q10_Tb1p28_b8_T36Ms_a0p9_20260503_155038`) qui encode tous les paramètres physiquement significatifs.
...
| `SRE_Projection_*.csv` | SRE projeté, M, SDF projetés (TCL empirique / K4-LogN / DSP) ; colonne `GEV_domaine` (gumbel/frechet/weibull_neg) si `METHODE_PROJECTION='gev_domaines'` |
...
Le fichier `mbd_simple-multi-process_v3_5.py` est mono-fichier autoporteur, organisé en 13 sections numérotées :
...
| 6 | KAPPA4 ANALYTIQUE | `_calculer_pwm`, `calculer_lmoments`, `_g_functions`, `_fit_loc_scale`, `_tau3_tau4_from_kh_analytic`, `ajuster_kappa4_pwm_analytic`, `_msdi_queue_droite`, `kappa4_rmse_msdi`, `kappa4_ppf` |
| 6bis | RAYLEIGH GÉNÉRALISÉE | `ajuster_rayleigh_gen`, `rayleigh_gen_ppf`, `rayleigh_gen_rmse_msdi`, dispatchers `ajuster_loi`/`loi_ppf` |
| 6ter | PROJECTION GEV 3 DOMAINES | `_gev_domaine`, `ajuster_gev_lmoments`, `calculer_projection_gev_domaines` (option `METHODE_PROJECTION`) |
| 7 | SRE / SDF DSP | `calculer_sre_analytique()` — Welch + NORMDEF [5.2] + Bendat |
