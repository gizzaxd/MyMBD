# -*- coding: utf-8 -*-
"""
MBD Kappa4 — Script mono-fichier autoporteur (V3.4)
====================================================
...
#   DELTA_F0: pas en Hz. num_f0 = (F0_MAX - F0_MIN)·DELTA_F0 + 1.
F0_MIN   = 1
F0_MAX   = 500
DELTA_F0 = 1
...
# PROBABILITE_CIBLE : probabilité de dépassement par "maximum" d'un bloc, si Cunnane désactivé.
#                     Plage (0,1) stricte. Si OPTION_CUNNANE=True, remplacée
#                     par p_eff = (N - a) / (N + 1 - 2a).
#                     Attention, probabilité de dépassement par "maximum" d'un bloc / quantile de tirage aléatoire (différent de la probabilité de dépassement pour "M" maxima calculé avec la TVE)
...
PROBABILITE_CIBLE = 0.9      # pris si OPTION_CUNNANE =False ; Probabilité de dépassement par "maximum" d'un bloc (différent de la probabilité de dépassement pour "M" maxima calculé avec la TVE)=>  0.9 <=> alpha=10%/0.1 ; 0.99 <=> alpha=1%/0.01 ; 0.999 <=> alpha=0.1%/0.001 
...
# ALFA_PROJECTION   : risque de dépassement projeté (= 1 − α dans [1]).
#                      0.9 = 90% non-dépassé. Plage 0.5–0.999.
ENABLE_PROJECTION  = True
DUREE_PROJECTION   = 36_000_000 #  36_000_000
ALFA_PROJECTION    = 0.90  # risque de dépassement projeté (= 1 − α dans [1]).
...
# OUTPUT_FOLDER     : dossier de sortie (créé si absent).
RANDOM_SEED         = 53
...
def kappa4_rmse_msdi(data, params):
    """Métriques de qualité de l'ajustement Kappa4 vs ECDF empirique.

    Référence : [1] §C.7 (p. 80) — tests KS-M et MSDI sur la queue droite.

    Métriques calculées :
        RMSE = √(mean((ECDF - CDF_K4)²))    — écart RMS sur tout l'échantillon
        MSDI = mean(ECDF - CDF_K4)          — biais signé (Mean Signed Diff)
        KS   = max|ECDF - CDF_K4|           — distance Kolmogorov-Smirnov
        pearson_r                            — corrélation ECDF / CDF_K4

    Note : la norme préconise un calcul sur les m plus grandes valeurs (m/n
    variable selon n, cf. [1] §C.7). Le code calcule ici sur tout l'échantillon
    pour rester généraliste ; à raffiner si besoin.
    ...
    try:
        if _kappa4_use_exact(params['k'], params['h']):
            ...
            cdf_v = scipy_kappa4(h=params['h'], k=params['k'],
                                 loc=params['xi'],
                                 scale=params['alpha']).cdf(sorted_d)
        diff  = ecdf - cdf_v
        rmse  = float(np.sqrt(np.mean(diff ** 2)))
        msdi  = float(np.mean(diff))
        try:
...
def rayleigh_gen_rmse_msdi(data, params):
    """RMSE / MSDI de l'ajustement Rayleigh généralisée vs ECDF empirique
    (analogue à kappa4_rmse_msdi : mute params avec 'ks' et 'pearson_r')."""
    if not params.get('success'):
        return np.nan, np.nan
    n = len(data)
    if n == 0:
...
def exporter_csv_projection(filepath, proj_results, meta,
    ...
    } for pr in sorted(proj_results, key=lambda x: x['f0'])]
    pd.DataFrame(rows).to_csv(filepath, index=False, sep=';', decimal=',')
...
def generer_html(filepath, results, sre_dsp, srx_low, srx_high, sdf_spectral,
    ...
    if has_proj: titles.append(f'SRE MBD Kappa 4 - Projeté (T={DUREE_PROJECTION:.0e}s, α={ALFA_PROJECTION})')
    ...
    if has_proj:
        ...
        fig.add_trace(go.Scatter(x=f0_list, y=sre_k4,   name='SRE MBD Kappa 4 - Temps du signal',
                                 line=dict(color='steelblue', width=1),
                                #  line=dict(color='royalblue', dash='dash', width=1),
                                 showlegend=False), row=row, col=1)
        fig.add_trace(go.Scatter(x=f0_list, y=sre_proj, name='SRE MBD Kappa 4 - Projeté',
                                 line=dict(color='crimson', width=2)), row=row, col=1)
        row += 1

    # Boutons d'échelle log/linéaire par graphique (X et Y indépendants).
    fig.update_layout(
        title=f"MBD — Analyse Spectrale  [{datetime.now().strftime('%Y-%m-%d %H:%M')}]",
...
def generer_html_details(filepath, results, sre_dsp, srx_low, srx_high,
    ...
    for h_val, color in [(-1.0, '#888888'), (0.0, '#4444aa'),
                         (1.0, '#44aa44'), (5.0, '#aa4444')]:
        xs_th, ys_th = _kappa4_tau_curve_for_h(h_val)
        if xs_th.size:
            fig.add_trace(go.Scatter(
                x=xs_th, y=ys_th,
                mode='lines', line=dict(color=color, width=1, dash='dot'),
                name=f'Kappa4 théorique h={h_val}', legendgroup='tau_theo',
            ), row=1, col=1)

    tau3_grid = np.linspace(-0.999, 0.999, 500)
    fig.add_trace(go.Scatter(
        x=tau3_grid, y=(5 * tau3_grid ** 2 + 1) / 6,
        mode='lines', line=dict(color='black', width=1),
        name='Borne τ4 = (5τ3²+1)/6', legendgroup='tau_theo',
    ), row=1, col=1)
...
def main(use_mp=False):
    ...
    num_f0     = int((F0_MAX - F0_MIN) * DELTA_F0 + 1)
        ...
        for r in tqdm(successful, desc="Projections"):
            ...
            for i, (params, maxima) in enumerate(zip(params_list, maxima_classes)):
                ...
                sre_p, M  = calculer_projection_lmoments(
                    params, maxima, n_blocs_i, total_blocs, TB, DUREE_PROJECTION, ALFA_PROJECTION)
    ...
    tag = (f"v3p2_f{F0_MIN:.0f}-{F0_MAX:.0f}"
    ...
    meta_json = os.path.join(OUTPUT_FOLDER, f"params_{tag}_{ts}.json")
    exporter_run_meta_json(meta_json, meta_run)

    sre_csv = os.path.join(OUTPUT_FOLDER, f"SRE_{tag}_{ts}.csv")
    exporter_csv_sre(sre_csv, all_results, sre_dsp, srx_low, srx_high,
                     f0_spectrum, meta=meta_run)

    if SDF_ENABLED:
        sdf_csv = os.path.join(OUTPUT_FOLDER, f"SDF_{tag}_{ts}.csv")
        exporter_csv_sdf(sdf_csv, all_results, sdf_spectral, f0_spectrum,
                         meta=meta_run)
        sdf_fit_csv = os.path.join(OUTPUT_FOLDER, f"SDF_Kappa4_Fit_{tag}_{ts}.csv")
        exporter_csv_sdf_kappa4_fit(sdf_fit_csv, all_results, meta=meta_run)

    if IID_GATE_ENABLED:
        iid_csv = os.path.join(OUTPUT_FOLDER, f"IID_QualityGate_{tag}_{ts}.csv")
        exporter_csv_iid(iid_csv, all_results, meta=meta_run)

    if proj_results:
        proj_csv = os.path.join(OUTPUT_FOLDER, f"SRE_Projection_{tag}_{ts}.csv")
    ...
    if KAPPA4_DEBUG_ANALYTIC:
        dbg_csv = os.path.join(OUTPUT_FOLDER, f"Kappa4_Debug_{tag}_{ts}.csv")
    ...
    html_path = os.path.join(OUTPUT_FOLDER, f"Rapport_{tag}_{ts}.html")
    config_info = {
    ...
    html_details = os.path.join(OUTPUT_FOLDER, f"Rapport_Details_{tag}_{ts}.html")
    ...
    logger.info("=" * 60)
    logger.info("  Terminé. Fichiers dans : %s/", OUTPUT_FOLDER)
    logger.info("=" * 60)
