...
> **Documented main program**: `mbd_simple-multi-process_v3_5.py`
> **Current version**: V3.5
> **Language**: Python 3.10+
> **Domain**: vibration testing — mechanical environment tailoring

> **V3.5 highlights** (formula audit against the documentation):
> - New **`METHODE_PROJECTION='gev_domaines'`** option: SRE projection via GEV
>   max-stability (h fixed to 0), discriminated over the 3 attraction domains
>   Gumbel / Fréchet / Negative Weibull (Colin 2023 §4.1) — see §4.10.
> - **One results subfolder per run** (`YYYYMMDD_HHMMSS_<file>_<parameters>`)
>   and analyzed file name (≤35 characters) in the HTML titles.
> - τ₃/τ₄ diagram: lower validity bound fixed, τ₄=(5τ₃²−1)/4 ([2] eq. 15);
>   h=−1 (GLO), h=0 (GEV), h=1 (GPD) curves labelled.
> - **MSDI fixed** per NF X50-144-3 §C.7 / CFM 2025 §3.1: *Mean Square
>   Deviation Index*, MSDI(%) = (100/m)·Σ[(X_exp − X_law)/X_exp]² over the m
>   points above the mean (right tail, Cunnane a=0.4 plotting positions). The
>   previous computation (signed mean ECDF−CDF gap, no square) did not match
>   the standard.
...
```
┌───────────────────────────────────────────────────────────────────┐
│  INPUT: CSV signal  (t, ẍ)  ─  fs ≈ 10,0 kHz typical              │
└───────────────────────────────────────────────────────────────────┘
              │
              ▼
┌────────────────────────────────────┐
...
### 4.10 Long-duration projection: F^M power method vs 3 GEV domains

Two SRE projection methods are available (`METHODE_PROJECTION`):

1. **`'puissance'`** (default, legacy) — direct raising of the selected law's
   CDF (`LOI_AJUSTEMENT`) to the power M:
   `SRE_proj = PPF(α^(1/M))` with `M = Occ(j)·T_v/T_b` ([1] §C.8-C.9, [2] eq. 35).

2. **`'gev_domaines'`** — alternative suggested by B. Colin ([2] §4.1): the
   local law is re-fitted with **h fixed to 0** (**GEV** family, sub-family of
   Kappa4; `g_r = r^(−k)·Γ(1+k)`, eq. 20.2). The shape k* is the root of the
   single equation `τ3_GEV(k) = t3` (1 unknown instead of the 2×2 (k,h)
   system — more stable), then (ξ*, α*) follow from eq. 33-34 with h=0.
   The projection uses the **exact max-stability of the GEV**, discriminated
   over the 3 Fisher-Tippett / Gnedenko attraction domains:

   | Domain | Condition | Asymptotic law of Z_sup | Projected parameters |
   |--------|-----------|-------------------------|----------------------|
   | EV1 | k ≈ 0 | **Gumbel** (light tail) | ξ_M = ξ + α·ln(M), α_M = α |
   | EV2 | k < 0 | **Fréchet** (heavy tail) | ξ_M = ξ + (α/k)(1 − M^−k), α_M = α·M^−k |
   | EV3 | k > 0 | **Negative Weibull** (bounded tail) | same as EV2 (continuous in k) |

   then `SRE_α = ξ_M + (α_M/k)·(1 − (−ln α)^k)` (Gumbel limit for k≈0).
   Strictly equivalent to `PPF_GEV(α^(1/M))` but in **closed form**: no
   α^(1/M) → 1 evaluation, hence no precision loss/clipping for very large M.
   The SRE *at the signal duration* still uses `LOI_AJUSTEMENT`; only the
   projection changes. The retained domain per f₀ (governing class) is
   exported in `SRE_Projection_*.csv` (`GEV_domaine` column); the Gumbel
   labelling threshold is `GEV_GUMBEL_K_TOL` (default 0.01, diagnostic only —
   the projection formula is continuous in k).
...
Edit the constants at the top of the file `mbd_simple-multi-process_v3_5.py` (SECTION 1), then:

```bash
python mbd_simple-multi-process_v3_5.py
```

For each run the program creates a **dedicated subfolder** inside `OUTPUT_FOLDER` (default `mbd_simple_output/`):

```
<YYYYMMDD_HHMMSS>_<file name ≤35 chars>_f<min>-<max>_Q<Q>_Tb<Tb>_b<b>_T<Tproj>_a<alfa>
e.g.: 20260612_231054_road_track_signal_f1-500_Q10_Tb1_b8_T36Ms_a0p9/
```

containing the CSVs, the parameter JSON sidecar and the HTML reports of that run. The analyzed file name (truncated to 35 characters) also appears in the HTML report titles and in the parameter box.
...
A short program that fully reuses `main()` from the main module: it generates an
excitation signal then runs the complete MBD computation with **exactly the same
visuals and outputs** (HTML reports, suffixed CSVs, JSON sidecar).
`python mbd_demo_v1.py` (config at the top of the file, sections A to D).

**All calculation options** of the main module are exposed and forwarded to the
pipeline (fitting law, Q/T_b, f₀ spectrum, Cunnane, SRX α, SDF, projection / GEV,
IID Quality Gate, multiprocess…). The demo **applies its overrides at module
level and registers the main module in `sys.modules`**, so `USE_MULTIPROCESS=True`
is safe even under *spawn* (Windows): the workers inherit every setting,
including `LOI_AJUSTEMENT`.

**Three signal families** (parameter `MODE_SIGNAL`):

| `MODE_SIGNAL` | Description | Dedicated parameters |
|---------------|-------------|----------------------|
| `'stationnaire'` | Flat PSD + ZMNL Hermite (CFM 2025 §4.3), driven by `KURTOSIS`/`SKEWNESS` (K=3 → Gaussian, K>3 → non-Gaussian) | `DUREE`, `RMS_CIBLE`, `KURTOSIS`, `SKEWNESS` |
| `'phases'` | Concatenation of stationary segments with distinct statistics → **block-wise** non-stationarity | `PHASES = [(duration, rms, kurt, skew), …]` |
| `'enveloppe'` | Base signal modulated by an RMS(t) envelope → **continuous** non-stationarity | `ENVELOPPE` (`'sinus'`/`'marche_alea'`), `RMS_MIN`, `RMS_MAX`, `PERIODE_ENV` |

A **standalone classification parameter block** (SECTION B: `N_CLUSTERS`,
`AUTO_SELECT_K`, `K_RANGE`, `MIN_SAMPLES_PER_CLUSTER`, `FEATURE_FLAGS`) is
reserved for **non-stationary signals**: in `'phases'`/`'enveloppe'` mode, set
`N_CLUSTERS` ≈ number of regimes (or `AUTO_SELECT_K=True`) and enable the
discriminating features (`variance`/`rms`, `kurtosis`/`crest_factor`,
`dominant_freq`…). In `'stationnaire'` mode, keep `N_CLUSTERS=1`.

On each run, all the main-module output lands in its own **timestamped run
subfolder** (see §5.3); the demo additionally stores the **generated signal**
there (one unique CSV per run) plus a **`RESUME_demo.txt`** (run config, achieved
RMS/skew/kurtosis statistics — global and per phase — and the list of produced
files), then prints a console summary (SRC/SRE peaks and IID verdict).
...
| `DELTA_F0` | float | 1 | **Frequency step (Hz)**. `num_f0 = round((F0_MAX-F0_MIN)/DELTA_F0) + 1`. E.g. 0.5 → twice as many points. (Fixed in V3.5: the old formula multiplied by ΔF0.) |
...
| `ALFA_PROJECTION` | float | 0.9 | Projected **non-exceedance** probability (the standard's risk α = 1 − this value). 0.9 = 90% non-exceeded. Range **0.5–0.999** |
| `METHODE_PROJECTION` | str | `'puissance'` | `'puissance'` = F^M on the `LOI_AJUSTEMENT` law (legacy); `'gev_domaines'` = GEV max-stability at h=0, Gumbel/Fréchet/Neg. Weibull discrimination ([2] §4.1) — see §4.10 |
| `GEV_GUMBEL_K_TOL` | float | 0.01 | (expert) \|k*\| threshold for the `gumbel` domain label — diagnostic only, the projection formula is continuous in k |
...
All in the **run subfolder** `OUTPUT_FOLDER/<YYYYMMDD_HHMMSS>_<file ≤35 chars>_<run info>/` (one folder per run, see §5.3), suffixed with a **descriptive tag** (e.g. `v3p5_f1-500_Q10_Tb1p28_b8_T36Ms_a0p9_20260503_155038`) that encodes all physically significant parameters.
...
| `SRE_Projection_*.csv` | Projected SRE, M, projected SDF (empirical CLT / K4-LogN / PSD); `GEV_domaine` column (gumbel/frechet/weibull_neg) if `METHODE_PROJECTION='gev_domaines'` |
...
The file `mbd_simple-multi-process_v3_5.py` is a self-contained single file, organized into 13 numbered sections:
...
| 6 | ANALYTICAL KAPPA4 | `_calculer_pwm`, `calculer_lmoments`, `_g_functions`, `_fit_loc_scale`, `_tau3_tau4_from_kh_analytic`, `ajuster_kappa4_pwm_analytic`, `_msdi_queue_droite`, `kappa4_rmse_msdi`, `kappa4_ppf` |
| 6bis | GENERALIZED RAYLEIGH | `ajuster_rayleigh_gen`, `rayleigh_gen_ppf`, `rayleigh_gen_rmse_msdi`, dispatchers `ajuster_loi`/`loi_ppf` |
| 6ter | 3-DOMAIN GEV PROJECTION | `_gev_domaine`, `ajuster_gev_lmoments`, `calculer_projection_gev_domaines` (option `METHODE_PROJECTION`) |
| 7 | PSD SRE / SDF | `calculer_sre_analytique()` — Welch + NORMDEF [5.2] + Bendat |
